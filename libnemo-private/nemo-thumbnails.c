...
nemo_get_thumbnail_frame (void)
{
	...
	if (thumbnail_frame == NULL) {
		thumbnail_frame = gdk_pixbuf_new_from_resource ("/org/nemo/icons/thumbnail_frame.png", NULL);
	}
...
ensure_filmholes (void)
{
	if (filmholes_left == NULL) {
		filmholes_left = gdk_pixbuf_new_from_resource ("/org/nemo/icons/filmholes.png", NULL);
	}
...
void
nemo_thumbnail_frame_video (GdkPixbuf **pixbuf)
{
	int width, height;
